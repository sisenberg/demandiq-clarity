...
export function scoreAllFactors(snapshot: EvaluateIntakeSnapshot): FactorScoringResult {
  ...
  const layerSummaries: LayerSummary[] = layers.map(layer => {
    ...
    const applicable = layerFactors.filter(f => f.applicable);
...
function makeResult(def: FactorDefinition, overrides: Partial<ScoredFactor>): ScoredFactor {
  return {
    factor_id: def.id,
    definition: def,
...
const FACTOR_SCORERS: Record<string, FactorScorer> = {
  ...
  gate_benchmark_data: (def, snap) => {
    ...
    return makeResult(def, {
      ...
      narrative: met
        ...
        : `Missing benchmark prerequisites: ${[!hasInjury && "injury data", !hasJurisdiction && "jurisdiction", !hasBilling && "billing data"].filter(Boolean).join(", ")}.`,
  ...
  injury_severity_class: (def, snap) => {
    if (snap.injuries.length === 0) {
      return makeResult(def, { raw_input: "0 injuries", score: 0, applicable: false, inapplicable_reason: "No injuries", narrative: "No injuries documented.", confidence: "low" });
    }
  ...
  treatment_invasiveness: (def, snap) => {
    let score = 1; // baseline: some treatment
  ...
  treatment_duration: (def, snap) => {
    ...
    if (dates.length < 2) {
      return makeResult(def, { raw_input: "Insufficient dates", score: dates.length === 1 ? 1 : 0, direction: "neutral", narrative: "Insufficient treatment dates to assess duration.", applicable: dates.length > 0, inapplicable_reason: dates.length === 0 ? "No treatment dates" : null, confidence: "low" });
    }
  ...
  gap_quality: (def, snap) => {
    const gaps = snap.upstream_concerns.filter(c => c.category === "gap");
    if (gaps.length === 0) {
      return makeResult(def, { raw_input: "No gaps", score: 5, direction: "neutral", narrative: "No treatment gaps to assess.", applicable: false, inapplicable_reason: "No gaps identified", confidence: "high" });
    }
  ...
  property_damage_context: (def, _snap) => {
    return makeResult(def, {
  ...
  causation_apportionment: (def, snap) => {
    ...
    if (total === 0) return makeResult(def, { raw_input: "No injuries", score: 5, direction: "neutral", narrative: "No injuries to assess causation.", applicable: false, inapplicable_reason: "No injuries", confidence: "low" });
  ...
  comparative_negligence: (def, snap) => {
    ...
    if (pct === null) return makeResult(def, { raw_input: "Not assessed", score: 0, direction: "neutral", narrative: "Comparative negligence not assessed.", applicable: false, inapplicable_reason: "No comparative negligence data", confidence: "high" });
    return makeResult(def, {
      raw_input: `${pct}%`,
      score: pct,
  ...
  venue_jurisdiction_adjustment: (def, snap) => {
    ...
    return makeResult(def, {
      ...
      score: 1, // default 1.0x multiplier
      direction: "neutral",
      narrative: state
