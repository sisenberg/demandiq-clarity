...
const ExportsPage = () => {
  const exportable = mockCases.filter(
    (c) => c.status === CaseStatus.Approved || c.status === CaseStatus.Exported
  );
    ...
    <div className="p-6 max-w-6xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Case packages ready for export
        </p>
      </div>
      ...
      {exportable.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {exportable.map((c) => (
            <div
              key={c.id}
              className="border border-border rounded-lg bg-card px-4 py-3 flex items-center justify-between"
            >
              <div className="flex items-center gap-3">
                <Package className="h-5 w-5 text-[hsl(var(--status-approved))]" />
                <div>
                  <p className="text-sm font-medium text-foreground">{c.title}</p>
                  <p className="text-xs text-muted-foreground">{c.caseNumber} · {c.claimant}</p>
                </div>
              </div>
              <span className="status-badge-approved">{c.status === CaseStatus.Exported ? "Exported" : "Ready"}</span>
            </div>
          ))}
        </div>
      )}
