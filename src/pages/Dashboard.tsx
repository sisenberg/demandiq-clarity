      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card-elevated overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-card-foreground">Recent Cases</h2>
          </div>
            ...
            {recentCases.length === 0 ? (
              ...
              recentCases.map((c) => (
                <div key={c.id} className="px-5 py-3 hover:bg-accent/30 transition-colors">
                  <p className="text-sm font-medium text-card-foreground">{c.title || `${c.claimant} v. ${c.insured}`}</p>
        ...
        <div className="card-elevated overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-card-foreground">Recent Documents</h2>
          </div>
          <div className="divide-y divide-border">
            {recentDocs.length === 0 ? (
              <div className="px-5 py-8 text-center">
              ...
              recentDocs.map((d) => (
                <div key={d.id} className="px-5 py-3 hover:bg-accent/30 transition-colors">
                  <p className="text-sm font-medium text-card-foreground">{d.file_name}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
